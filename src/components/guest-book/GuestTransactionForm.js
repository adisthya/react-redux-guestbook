...
import { GUEST_ACTIONS, rate } from '../../reducers/guest-reducer';
...
function GuestTransactionForm(props) {
  const { guests, transactions, trxId, checkIn, checkOut, switchPage, resetSelectedTrx } = props;
...
function mapDispatchToProps(dispatch) {
  return {
    checkIn: (payload) => dispatch({ type: GUEST_ACTIONS.CHECK_IN, payload }),
    checkOut: (payload) => dispatch({ type: GUEST_ACTIONS.CHECK_OUT, payload })
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(GuestTransactionForm);
